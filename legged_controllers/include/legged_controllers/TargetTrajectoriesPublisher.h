...
public:
    ...
    , tf2_(buffer_)
  {
    ...
    auto goalCallback = [this](const geometry_msgs::PoseStamped::ConstPtr& msg) {
      std::cout << "received goal" 
        << msg->pose.position.x << " " << msg->pose.position.y << " " << msg->pose.position.z << std::endl
    ...
    auto cmdVelCallback = [this](const geometry_msgs::Twist::ConstPtr& msg) {
      // std::cout << "cmdVelCallback" << msg->linear.x << " " << msg->linear.y << " " << msg->angular.z 
      //   << msg->angular.y << " " << msg->angular.x
      //   << std::endl;
      ...
      vel_msg.linear.x = lastVel_[0];
      vel_msg.linear.y = lastVel_[1];
      vel_msg.linear.z = lastVel_[2];
      vel_msg.angular.z = lastVel_[3];
      vel_pub_.publish(vel_msg);

      // ROS_INFO_STREAM("lastVel_: "
      //           << "linear.x=" << lastVel_[0]
      //           << ", linear.y=" << lastVel_[1]
      //           << ", linear.z=" << lastVel_[2]
      //           << ", angular.z=" << lastVel_[3]);
      
    

      ocs2_msgs::mpc_observation obs_msg;
      obs_msg = ros_msg_conversions::createObservationMsg(latestObservation_);
      obs_pub_.publish(obs_msg);

      // ROS_INFO_STREAM("latestObservation_: "
      //           << "time=" << latestObservation_.time
      //           << ", state=(");
      // for (size_t i = 0; i < latestObservation_.state.rows(); ++i) {
      //     if (i > 0) ROS_INFO_STREAM(", ");
      //     ROS_INFO_STREAM(latestObservation_.state(i));
      // }
      // ROS_INFO_STREAM("), input=(");
      // for (size_t i = 0; i < latestObservation_.input.rows(); ++i) {
      //     if (i > 0) ROS_INFO_STREAM(", ");
      //     ROS_INFO_STREAM(latestObservation_.input(i));
      // }
      // ROS_INFO_STREAM("), mode=" << latestObservation_.mode);
